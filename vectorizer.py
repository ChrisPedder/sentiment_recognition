...
class BoWVectorizer:
    ...
    def fit(self, dataset, min_occurrences=5):
        ...
        for batch in dataset:
            cleaned_text = self._prepare_batch(batch)
    ...
    def vectorize(self, batch):
        ...
        cleaned_text_list = self._prepare_batch(batch)
...
class BERTVectorizer(object):
    ...
    Note that I only return this CLS-embedding and then feed straight to the
    logistic regression as a feature vector, which is a bit crap honestly.
    A better approach would be to at least train a three class classification
    in Tensorflow, but I ran out of time, and Huggingface seems to be down now
    anyway. Sad face, not Huggingface...
    """
    ...
    def vectorize(self, batch):
        sentences = self._prepare_batch(batch)
